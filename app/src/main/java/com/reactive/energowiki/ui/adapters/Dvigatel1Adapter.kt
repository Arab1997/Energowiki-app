...
class Dvigatel1Adapter(private val listener: (Any) -> Unit): BaseAdapter<Any>(R.layout.item_calculator){
    override fun bindViewHolder(holder: ViewHolder, data: Any) {
        when (data) {
            ...
            8 -> holder.itemView.name.text = "Площадь поперечного сечения"
            9 -> holder.itemView.name.text = "Площадь поперечного сечения"
